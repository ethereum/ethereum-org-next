...
import { commonInputTriggerStyles, defineMergeStyles, radioDefaultTheme } from "./components.utils"
...
const { commonContainerProps, commonControlProps, commonLabelProps } = commonInputTriggerStyles

const baseStyleContainer = defineStyle({ ...commonContainerProps })

const baseStyleControl = defineMergeStyles(radioDefaultTheme.baseStyle?.({} as never).control, commonControlProps, {
  boxSize: 4,
  fontSize: "md",
  "*:hover > &": {
    outlineOffset: "-1px",
